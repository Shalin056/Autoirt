"""
run_yn_vocab_nlp_experiment_fullscale.py
============================================

Same idea as run_yn_vocab_nlp_experiment.py, but at the real DET item
bank scale from config.py (3,290 items, 25,000 sessions) instead of
the small-scale stand-in settings (150 items, 6,000 sessions) used for
every check so far. Single run first, not replicated -- matches the
"verify one run before paying for replication" rule used everywhere
else in this project, and full-scale replications are expensive (the
synthetic-feature full-scale run took roughly 150 minutes per pass, see
run_det_experiment_replicated_fullscale.py's own notes -- real-feature
generation adds some cost on top of that, not yet measured).

Run with:
    python scripts/run_yn_vocab_nlp_experiment_fullscale.py
...
N_ITEMS = config.N_YN_VOCAB_ITEMS
N_SESSIONS = config.N_DET_SESSIONS
ITEMS_PER_SESSION = config.YN_ITEMS_PER_SESSION
...
# Full-scale SYNTHETIC-feature baseline (det_replication_summary_fullscale.csv,
# 3 replications, already established this project) -- the "before" comparison.
SYNTHETIC_BASELINE_PEARSON = {"Cold": 0.758, "Jump 20": 0.798, "Jump 40": 0.717,
                               "Jump 80": 0.858, "Warm 05-08": 0.847, "Warm 05-15": 0.847}

RESULTS_PATH = os.path.join(os.path.dirname(__file__), "..", "results",
                             "yn_vocab_nlp_experiment_fullscale.csv")
...
def build_conditions(random_seed: int):
    """Builds items/sessions/responses at full DET scale and returns the
    6 (split_name, train, test) tuples."""
    print(f"Generating {N_ITEMS} real-content Y/N Vocab items...")
    t0 = time.time()
...
def main():
    ...
    print("\n" + "=" * 110)
    print("SUMMARY: real NLP features (full DET scale) vs. paper vs. synthetic-feature baseline")
    print("=" * 110)
    ...
    print("\nThis is a single run at full scale, not replicated. Same rule as everywhere else:")
    print("do not trust these numbers as a final answer until replicated, or at minimum treat")
    print("them as a first look, given how much the small-scale Warm/Jump-start gains changed")
    print("once actually replicated.")
