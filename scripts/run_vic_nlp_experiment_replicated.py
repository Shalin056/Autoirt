"""
run_vic_nlp_experiment_replicated.py
=======================================

ViC counterpart to run_yn_vocab_nlp_experiment_replicated.py, for the
same reason: the single run looked uniformly positive (including a
striking +0.113 gain on Cold-start), but Y/N Vocab's single run looked
the same way and mostly didn't survive replication. Don't trust either
without checking across seeds.

COST: single run took 14.2 minutes (smaller item bank than Y/N Vocab).
5 replications is roughly 70-75 minutes -- run unattended.

Run with:
    python scripts/run_vic_nlp_experiment_replicated.py

Individual condition rows are appended to results/vic_nlp_experiment.csv;
the aggregated summary is saved separately to
results/vic_nlp_experiment_replicated_summary.csv.
"""
...
from run_vic_nlp_experiment import (
    run_one_seed, append_result_row, PAPER_PEARSON, SYNTHETIC_BASELINE_PEARSON,
)

N_REPLICATIONS = 5
BASE_SEED = 4000  # distinct from every other seed used in this project (42, 500, 1000, 3000, 9000)

SUMMARY_PATH = os.path.join(os.path.dirname(__file__), "..", "results",
                             "vic_nlp_experiment_replicated_summary.csv")
...
def print_summary_table(summary_rows: list):
    ...
    print("\n" + "=" * 110)
    print(f"SUMMARY OVER {N_REPLICATIONS} REPLICATIONS (real ViC NLP features, full MCEM pipeline)")
    print("=" * 110)
    ...
    print("\nTrust 'gain (min)', not single-run numbers or even the mean. Positive min = every")
    print("replication beat the baseline, a real effect. Straddling zero = noise.")
